...
app.get('/fit', async (req, res) => {
    ...
    // TODO anme, town, maxlenth
    try {
        const template = `SELECT name as campground, location, maxlength FROM campgrounds WHERE maxlength > $1`;
        const response = await pool.query(template, [searchTerm]);
        const results = response.rows.map((row) => {return (row)})
        res.json({campgrounds: results});
    } catch (err){
...
app.get('/elevation', async (req, res) => {
    ...
    try {
        const response = await pool.query(query, [altitude]);
        const results = response.rows.map((row) => {return (row)})
        res.json({campgrounds: results});
...
app.post("/addCampground", async (req,res) => {
    //name, location, maxlength, elevation, sites
    let name = req.body.name;
    let location = req.body.location;
    let maxlength = req.body.maxlength;
    ...
    try {
        ...
        if (check.rowCount != 0){
            ...
            // else let's insert it
            const template2 = "INSERT INTO campgrounds (name, location, maxlength, elevation, sites) VALUES ($1, $2, $3, $4, $5)"
            const response = await pool.query(template2, [name, location, maxlength, elevation, sites]);
            res.json({status: "added"});
        }
    } catch (err){
        // whoops
