...
app.get('/search', (req, res) => {
    ...
    let results = []
    // TODO name. lengthlimit, elevation
    for( var i = 0; i < campgrounds.length; i++){
        if(searchTerm == campgrounds[i].name){
            results.push({name: campgrounds[i].name, lengthLimit: campgrounds[i].lengthLimit,
            elevation: campgrounds[i].elevation});
...
app.get('/fit', (req, res) => {
    ...
    // TO DO
    let fit = req.query.length;
    let results = []

    for(var i = 0; i < campgrounds.length; i++){
        if(campgrounds[i].lengthLimit > fit){
            results.push({campground: campgrounds[i].name, location: campgrounds[i].name,
            maxLength: campgrounds[i].lengthLimit})
        }
    }
...
*/
  ...
  app.get('/elevation', (req,res) => {
      ...
      for(var i = 0; i < campgrounds.length; i++){
          if(direction == 'higher' && campgrounds[i].elevation > altitude){
              results.push({campground: campgrounds[i].name, elevation: campgrounds[i].elevation, town: campgrounds[i].town})

          }
          if(direction == 'lower' && campgrounds[i].elevation < altitude){
            results.push({campground: campgrounds[i].name, elevation: campgrounds[i].elevation, town: campgrounds[i].town})
